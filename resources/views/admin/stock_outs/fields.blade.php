...
<div class="form-group col-sm-6">
    ...
    {!! Form::text('kode',$stockOut->kode, ['class' => 'form-control','readonly']) !!}
</div>
@else
<div class="form-group col-sm-6">
...
<div class="form-group col-sm-6">
    {!! Form::label('tgl', 'Tgl:') !!}
    {!! Form::date('tgl', $stockOut->tgl, ['class' => 'form-control']) !!}
</div>
@else
<div class="form-group col-sm-6">
    {!! Form::label('tgl', 'Tgl:') !!}
    ...
    <div class="box-body table-responsive no-padding"  >
      <table class="table table-bordered" id="crud_table" border="3">
          ...
          <tr class="trbody">
            ...
            @if($action == 'edit')
            <td >
            {!! Form::date('row[0][tgl]',$stockOut->tgl,['class'=>'form-control tgl  ','id'=>'tgl'])!!}
            </td>
            @else
             <td >
                    ...
                    $.ajax({
                        ...
                        success: function(data) {
                            response($.map(data,function(item){
                              return{
                                value:item.value+","+item.kode+",Ketersediaan:"+item.stock_akhir,
                                nama:item.value,
                                id_stock:item.id,
     ...
     $('#add').click(function(){
      ...
      var html_code = `<tr id='row`+count+`' class='trbody'>`;
       ...
       html_code += `@if($action == 'edit')
            <td >
            {!! Form::date('row[`+count+`][tgl]',$stockOut->tgl,['class'=>'form-control tgl  ','id'=>'tgl'])!!}
            </td>
            @else
             <td >
...
// <!-- auto hitung -->
    ...
    //validasi
     $('#crud_table').on("focusout", ".form-control.nama", validasiNama);
     $('#crud_table').on("focusout", ".form-control.jml", validasiJml);

     function validasiNama(){
         $(this).closest('#crud_table').find('tr.trbody').each(function() {
              var row =$(this);
              //cek Ketersediaan
              var avail = row.find(".form-control.stock_akhir").val();
              var name = row.find(".form-control.nama").val();
              console.log('jl');
              if(avail < 1){
                    $('.error').append(`<span class="cek" ><p style="color:red;">Stock Tidak tersedia pada `+name+`</p></span>`);
              }
              if(avail > 1){
                $( '.cek' ).remove();
                }
              ...
              var row =$(this);
               ...
               var name = row.find(".form-control.nama").val();
                var value = row.find(".form-control.jml").val();
                  ...
                  if(value > avail){
                    $('.error').append(`<span class="cek" ><p style="color:red;">Stock lebih dari stock Akhir pada `+name+` Ketersediaan Hanya `+avail+`</p></span>`);
                     }
                  if(value < avail){
                    $( '.cek' ).remove();
                    }
                  console.log('ceKJMl',value);
              });
     }
